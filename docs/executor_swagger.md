...
+kubebuilder:validation:XValidation:rule="[has(self.s3), has(self.git), has(self.http), has(self.artifactory), has(self.hdfs), has(self.raw), has(self.oss), has(self.gcs), has(self.azure), has(self.plugin)].filter(x, x).size() == 1",message="exactly one artifact location must be specified"
...
| tasks | [][DAGTask](#d-a-g-task)| `[]*DAGTask` |  | | Tasks are a list of DAG tasks</br>+patchStrategy=merge</br>+patchMergeKey=name</br>+kubebuilder:validation:MinItems=1 |  |
...
| value | string| `string` |  | | Value is the value to be used in the operation with the metric's current value. If no operation is set,</br>value is the value of the metric</br>+kubebuilder:validation:MinLength=1 |  |
